    ...
    if ($("body").hasClass("page-template-c80-timeline")) {
        ...
        if (hash) {
            ...
            if (phasetoLoad !== null && phasetoLoad !== undefined) {
                ...
                var phaseNav = jQuery(
                    "li.fase-arrow.nav-item#navfase-" + phasetoLoad + " a"
                ).attr("href");
                $("html, body").animate(
                    {
                    ...
                    function () {
                        ...
                        $("#timeline-js-container")
                            .empty()
                            .append(
                                '<div class="loadingZone"><i class="fa fa-spin fa-circle-o-notch"></i></div>'
                            );
                        var timeline = window.setTimeout(function () {
                            startTimeline(
                                phasetoLoad,
                                nextPhase(fases, phasetoLoad)
                            );
                        }, 500);
        ...
        $(
            ".toggle-timeline, .gotophase, .gotophase-mobile, .header-presentacion-fase"
        ).on("click", function (e) {
            e.preventDefault();
            ...
            var timeline = window.setTimeout(function () {
                startTimeline(fase, nextfase);
            }, 1000);
            $("#timeline-active ul.fases-main li#navfase-" + fase).addClass(
                "running"
            );
        });
        ...
        $("body").on("click", "a.btn-nextphase, span.closetimeline", function (
            e
        ) {
            closeTimeline();
        });
        ...
        $(document).keydown(function (e) {
            if (e.which == 39) {
                ...
                if (next) {
                    console.log(next);
                    $("body").removeClass("timeline-on");
                    $("#timeline-active ul.fases-main li").removeClass(
                        "running"
                    );
                    $("html, body").animate(
                        {
        ...
        if (isMobile()) {
            $(document).click(function (event) {
                ...
                console.log($target);
                if (
                    !$target.closest("#timeline-nav").length &&
                    $("#timeline-nav").hasClass("active") &&
                    !$target.hasClass("toggle-timeline-nav") &&
                    !$target.hasClass("fa-bars") &&
                    !$target.hasClass("sharer-display") &&
                    !$target.hasClass("share-link")
                ) {
    ...
    jQuery.getJSON(c80.timelineurl, function (data) {
        ...
        timelineObj.on("change", function (data) {
            ...
            if (data.unique_id == lastSlide.data.unique_id) {
                ...
                var next = $(
                    'section.presentacion-fase[data-fase="' + nextfase + '"]'
                ).attr("id");
                var nextText = $(
                    '#timeline-nav li a[href="#' + next + '"] h1'
                ).text();

                if (next) {
                    if (isMobile()) {
                        jQuery("div.timeline-mobile-header").addClass(
                            "shownext"
                        );
                        jQuery("div.nextphase-mobile").addClass("active");
                        jQuery("div.nextphase-mobile a").attr(
                            "href",
                            "#" + next
                        );
                        jQuery("div.nextphase-mobile a span.range").text(
                            ": " + nextText
                        );
                        jQuery("div.mobile-interaction")
                            .empty()
                            .append(
                                '<a class="btn-nextphase" data-toggle="nextphase" href="#' +
                                    next +
                                    '"><span class="msg">Próximo período</span> <i class="fa fa-angle-right"></i></a>'
                            );
                    } else {
                        jQuery(".tl-storyslider").append(
                            '<a class="btn-nextphase" data-toggle="nextphase" href="#' +
                                next +
